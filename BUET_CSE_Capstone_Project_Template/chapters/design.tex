...
To tackle the complexities of asymmetric multi-turn dialogue, the system is designed as a modular, multi-tier computational pipeline. The overall functional architecture decomposes the problem into four primary subsystems:
\begin{enumerate}
...
\begin{figure}[htbp]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/tikz/system_architecture.tikz}}
  \caption{Overall functional system architecture, illustrating data ingestion across three corpora, shared utterance encoding, the dual-stream Antiphony intention network, and the 19-member multi-task strategy classification framework with calibrated decision blending.}
...
\begin{figure}[htbp]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/tikz/strategy_annotation_pipeline.tikz}}
...
\textbf{Annotator Granularity Effects:} The corpus was annotated across four distinct segments: \texttt{shimanto} (turns 1--2000), \texttt{shovon} (turns 2001--4000 and 6001--7000), an unassigned batch (turns 4001--6000), and \texttt{abhishek} (turns 7001--10600). Analysis reveals significant variance in annotation granularity: \texttt{shimanto} averaged $2.384$ labels per turn ($9.0\%$ empty sets, $66.8\%$ multi-label), whereas \texttt{abhishek} averaged $1.240$ labels per turn ($13.2\%$ empty sets, $30.8\%$ multi-label). This variance represents a rater effect in label granularity rather than an intrinsic linguistic shift in the underlying conversations.
...
\begin{figure}[htbp]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/tikz/intention_annotation_passes.tikz}}
...
\begin{figure}[htbp]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/tikz/datagen_pipeline.tikz}}
...
\begin{figure}[htbp]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/tikz/strategy_model.tikz}}
...
The centerpiece intention architecture of this project is the \textit{AntiphonyClassifier} (v14). Built to overcome the limitations of flat sequence transformers and symmetric dialogue encoders, Antiphony explicitly embeds the fundamental asymmetry of persuasion into its neural wiring.

\begin{figure}[p]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/tikz/antiphony_arch.tikz}}
  \caption{The AntiphonyClassifier (v14) neural architecture: raw turns undergo batched utterance encoding, masked mean-pooling, and role-pure splitting ($R=20$). Persuader and Decider streams pass through independent 2-layer Transformers. The Decider stream queries the Persuader stream via directional cross-attention. Contextualized Decider states are aggregated in parallel by a commitment-trajectory GRU (read at the true final Decider turn) and attention pooling, followed by latent concatenation and Class-Balanced Focal classification.}
...
\begin{enumerate}
  ...
  \item \textbf{Directional Asymmetric Cross-Attention:} To model how decider stance evolves in response to persuader arguments, the Decider stream acts as the Query, while the Persuader stream provides the Keys and Values:
    ...
    \end{equation}
    where $\mathbf{M}_{\mathcal{P}}$ is the key-padding mask. The cross-attended contextual representations are combined via a residual connection and layer normalization:
    \begin{equation}
      \mathbf{X} = \text{LayerNorm}(\widetilde{\mathbf{H}}^{(\mathcal{D})} + \mathbf{C}^{(\mathcal{D} \to \mathcal{P})}) \in \mathbb{R}^{B \times R \times d}
    \end{equation}
  \item \textbf{Parallel Trajectory and Pooling Aggregators:} The contextualized decider sequence $\mathbf{X}$ is aggregated via two complementary mechanisms operating in parallel:
    \begin{itemize}
      \item \textit{Commitment-Trajectory GRU:} A single-layer GRU captures temporal momentum:
        \begin{equation}
          \mathbf{h}_t = \text{GRU}(\mathbf{x}_t, \mathbf{h}_{t-1})
        \end{equation}
        Crucially, the trajectory vector $\mathbf{h}_T$ is extracted using PyTorch \texttt{pack\_padded\_sequence} at each dialogue's \textit{true last valid decider turn}, completely bypassing trailing padding slots.
...
\clearpage
\section{Architectural Evolution: The Path to v14}
\label{sec:evolution}

The AntiphonyClassifier was not designed in isolation; it emerged from a systematic 14-version empirical investigation across four distinct evolutionary stages, documented in \Cref{fig:architectural_evolution}.

\begin{figure}[htbp]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/tikz/architectural_evolution.tikz}}
  \caption{Four-stage architectural evolution of intention classification models (v1 through v14), depicting key innovations, critical bug diagnoses, and peak test Macro-F1 across stages.}
  \label{fig:architectural_evolution}
\end{figure}

\subsection{Stage 1 (v1 to v5): Monolithic Flat Baselines and Forensic Debugging}
\begin{itemize}
  \item \textbf{v1 (Flat Base Model):} Concatenated dialogues truncated to 256 tokens with \texttt{[CLS]} pooling and independent binary and modifier heads. Reached $0.68$ Macro-F1 with RoBERTa, but DeBERTa collapsed to the majority class and modifier heads completely failed on rare classes.
  \item \textbf{v2 (Speaker-Aware Encoding):} Added learnable role embeddings ($\text{Persuader}=0, \text{Persuadee}=1$) prior to masked mean-pooling. Slightly improved recall ($0.70$ Macro-F1), but remained bottlenecked by 256-token truncation.
  \item \textbf{v3 (Augmentation Collapse and Diagnostic Audit):} Added EDA text augmentation \cite{wei2019eda} and raw inverse-frequency class weighting. Training completely collapsed. Forensic auditing identified two critical bugs: (i) checkpoint selection was mistakenly scored on \texttt{pos\_label="yes"} F1, allowing an all-yes trivial model to score $1.0$ and be saved, and (ii) raw inverse-frequency weights on the 13-example \texttt{conditional} class caused catastrophic gradient spikes.
  \item \textbf{v4 (Metric Fix and Multi-Objective Loss):} Fixed the evaluation harness to use true unweighted Macro-F1; implemented Class-Balanced Focal Loss ($\beta=0.999$); cascaded the binary prediction into the modifier head; added in-batch Supervised Contrastive Loss. Training stabilized, but performance plateaued at $0.649$ Macro-F1.
  \item \textbf{v5 (Augmentation Formatting Forensics):} An audit revealed that $57.5\%$ of speaker markers (\texttt{[Persuader]}) in raw transcripts were glued to newlines without spaces. Standard whitespace augmentation tokenizers fragmented these tags, corrupting role markers in $15.9\%$ of augmented instances. Implementing regex-based tokenization and Persuadee-biased mutation weights resolved the corruption, but Macro-F1 remained capped ($0.670$), proving that flat 256-token architectures had reached their theoretical ceiling.
\end{itemize}

\subsection{Stage 2 (v6 to v9): Hierarchical Discourse Modeling}
\begin{itemize}
  \item \textbf{v6 (Hierarchical Dialogue Transformer):} The foundational breakthrough: turns encoded individually ($32 \times 64 = 2{,}048$ token capacity) via a shared encoder, pooled into turn vectors, combined with role and positional embeddings, and processed by a 4-layer inter-turn dialogue transformer with learned attention pooling. Macro-F1 surged to \textbf{$0.793$} (RoBERTa), \textbf{$0.797$} (DeBERTa), and \textbf{$0.807$} (TOD-BERT).
  \item \textbf{v7 (Back-Translation on Flat Model -- Negative Control):} Applied French back-translation to the flat v5 model as an experimental control; Macro-F1 dropped to $0.590$--$0.650$, proving that data augmentation cannot overcome architectural truncation.
  \item \textbf{v8 (Hierarchical Back-Translation):} Combined the v6 hierarchical architecture with back-translation. Binary Macro-F1 advanced to \textbf{$0.821$} (DeBERTa), but modifier F1 fell, revealing that translation round-trips wash away subtle hedge words.
  \item \textbf{v9 (Tri-Model Ensemble and Monte Carlo Uncertainty):} Soft probability averaging across saved v6 checkpoints (RoBERTa, DeBERTa, TOD-BERT) achieved $0.818$ Macro-F1. 20 stochastic Monte Carlo dropout forward passes demonstrated that prediction variance was significantly lower on correct instances ($0.0220$) than incorrect instances ($0.0297$), providing a calibrated uncertainty signal.
\end{itemize}

\subsection{Stage 3 (v10 to v13): Recency Gating and Specialist Routing}
\begin{itemize}
  \item \textbf{v10 (Last-Persuadee-Turn Gating):} Added a sigmoid gate explicitly anchoring the pooled representation to the final Persuadee hidden state, initialized with a $+3.0$ bias to prevent early deflection. Achieved \textbf{$0.834$} Macro-F1 with RoBERTa, establishing the peak single-model binary score of the two-headed lineage.
  \item \textbf{v11 (Multi-Pivot Minority Augmentation):} Implemented French and German back-translation under beam search and random sampling; DeBERTa reached \textbf{$0.829$}, but modifier F1 continued to degrade.
  \item \textbf{v12 (Specialist Routing):} Decoupled the tasks without retraining: routing binary classification to v10 Gated RoBERTa ($0.834$) and modifier classification to v6 Plain DeBERTa ($0.452$).
  \item \textbf{v13-Solo (Modifier Removal):} Deleted the modifier head entirely, dedicating all capacity to a 2-layer binary head. Reached \textbf{$0.825$} on second-pass labels and \textbf{$0.8359$} on third-pass labels.
\end{itemize}

\subsection{Stage 4 (v14): Antiphony Dual-Stream Architecture}
v14 implemented the speaker-disentangled dual-stream cross-attention architecture, establishing project-wide peak performance ($0.8623$ on second pass, $0.8446$ on third pass), definitively proving the necessity of modeling asymmetric conversational dynamics.
...
Inference on a complete 20-turn dialogue executes in approximately $45$\,ms on an NVIDIA T4 GPU ($380$\,ms on an 8-core Intel CPU under batch size 1). Measured GPU training runtimes across versions: v4 ($\approx 20$\,min), v6 ($\approx 2.5$\,h), v8 ($\approx 5.0$\,h), v10 ($\approx 2.5$\,h), v13 ($\approx 3.5$\,h), and v14 ($\approx 3.0$\,h). The category clean rerun took 177 minutes, and strategy ensemble search required $\approx 4.0$\,hours.
