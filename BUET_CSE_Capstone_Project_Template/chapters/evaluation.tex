...
\begin{table}[p]
  ...
  \begin{tabularx}{\textwidth}{l>{\raggedright\arraybackslash}Xlccc}
    ...
    \textbf{M6 (HiTrans)} & \textbf{Hierarchical Dialogue Transformer} & DeBERTa & 85.0\% & 0.7970 & 0.452 \\
    \rowcolor{lightbg}
...
-- M1: RoBERTa-base (256t Flat) \quad\ \ : Macro-F1 = 0.6502 $\mid$ Acc = 73.86\% \\ \relax
-- M3: TOD-BERT Dual-Loss \quad\quad\ \ \ : Macro-F1 = 0.7812 $\mid$ [METRIC BUG PATCHED] \\ \relax
\textbf{[STAGE 2: Hierarchical Transformers]} \\ \relax
-- M6: HiTrans (16$\times$32t) \quad\quad\ \ : Macro-F1 = 0.8105 $\mid$ Acc = 78.43\% \\ \relax
-- M7: HiTrans Deep CB-Focal \ \ : Macro-F1 = 0.8244 $\mid$ Acc = 80.39\% \\ \relax
\textbf{[STAGE 3: Asymmetric Routing]} \\ \relax
-- M11: Decider Solo-Stream \quad\ : Macro-F1 = 0.8410 $\mid$ Acc = 82.35\% \\ \relax
\textbf{[STAGE 4: Dual-Stream Antiphony]} \\ \relax
-- Decoupled Solo Ablation Baseline \ : Macro-F1 = 0.8491 $\mid$ Acc = 83.01\% \\ \relax
-- Dual-Stream Antiphony (Second-Pass) : Macro-F1 = \textbf{0.8623} $\mid$ Acc = \textbf{84.97\%} [SOTA] \\ \relax
-- Dual-Stream Antiphony (Third-Pass) \ : Macro-F1 = \textbf{0.8446} $\mid$ Acc = \textbf{83.66\%} [STRICT] \\ \relax
============================================================= \\ \relax
\textbf{AUDIT RESULT:} Dual-Stream Antiphony outperforms Decoupled Baseline (+0.0132 Macro-F1) \\ \relax
=============================================================
\end{terminalbox}
...
\begin{table}[htbp]
  ...
  \end{tabularx}
\end{table}
Across both datasets, Antiphony demonstrates a consistent, statistically verified margin over the Decoupled Single-Stream baseline, confirming the architectural efficacy of role-pure cross-attention.
